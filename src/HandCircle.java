...
/**
 * Circles specifically intended to come from the hands. Constantly triggered,
 * but grow in size when the interactor brings their hands together.
 * 
...
public class HandCircle implements Circle {
	...
	private long startTime; // when the circle was first initialized (or if we are starting the circle over
							// after a stop)
	...
	public void update(float x, float y) {
		...
		if (System.currentTimeMillis() - startTime > stagger) {
			...
			if ((this.x == -1 || this.y == -1 || diam > maxDiam) && !(x < -100 && y < -100)) {
	 ...
	 * 
	 * @param larger
	 *            true if we want the circle to be the larger size, false if we want
	 *            the smaller size
	 */
