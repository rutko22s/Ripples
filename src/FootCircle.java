...
/**
 * Circles designed to come from the feet. Trigger only when the interactor's
...
public class FootCircle implements Circle {
	...
	public void update(float x, float y) {
		...
		if (System.currentTimeMillis() - startTime > stagger) {
			...
			if ((this.x == -1 || this.y == -1) // || currentDist-average > .01)
					|| (onFloor) && (diam >= MAX_DIAM) && !(x < -100 && y < -100)) {
				if (!stop) {
	...
	/**
	 * @param stop
	 *            True to stop the ripple, false to make it continue
	 */
